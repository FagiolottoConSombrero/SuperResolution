...
class Spec_SPAB(nn.Module):
    def __init__(self, in_channels, mid_channels=None, out_channels=None, bias=False):
        ...
        self.se_block = SEBlock(out_channels)  # Aggiunto SE Block
    ...
    def forward(self, x):
        ...
        out3 = self.c3_r(out2_act)
        out3 = self.se_block(out3)  # Applicazione del blocco di attenzione
...
class Spec_SPAN(nn.Module):
    ...
    def __init__(self, num_in_ch, num_out_ch, feature_channels=48, bias=True):
        ...
        self.conv_1 = Conv3XC(in_channels, feature_channels, gain1=2, s=1)
        self.block_1 = Spec_SPAB(feature_channels, bias=bias)
        self.block_2 = Spec_SPAB(feature_channels, bias=bias)

        self.conv_cat = conv_layer(feature_channels * 4, feature_channels, kernel_size=1, bias=True)
        self.conv_2 = Conv3XC(feature_channels, feature_channels, gain1=2, s=1)

        self.se_block = SEBlock(feature_channels)  # Aggiunto SE Block prima del layer finale
    ...
    def forward(self, x):
        ...
        out = self.se_block(out)  # Applicazione del SE Block
